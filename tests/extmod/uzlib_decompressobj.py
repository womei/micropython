...
# decompress using a decompressobj, feeding in 1 byte at a time
def decomp(data, wbits=15):
    d = zlib.decompressobj(wbits)
    out = []
    for b in data:
        out.append(d.decompress(bytes([b])))
    out.append(d.flush())
    return b''.join(out)
...
PATTERNS = [
    ...
    b'x\x9c3\x00\x00\x001\x001',
    b'x\x9cK\x04\x00\x00b\x00b',
    b'x\x9c30\xa0=\x00\x00\xb3q\x12\xc1',
...
# only raw streams implemented at the moment
#for p in PATTERNS:
#    print(decomp(p))
