...
Example usage::
   ...
   block = machine.ADCBlock(id, 12) # create an ADCBlock with 12-bit resolution
   adc = block.connect(4, pin)      # connect channel 4 to the given pin
   val = adc.read_v()               # read an analog value
...
.. method:: ADCBlock.connect(source)
.. method:: ADCBlock.connect(channel, source)

   Connect up a channel on the ADC peripheral so it is ready for sampling,
   and return a :ref:`machine.ADC <machine.ADC>` object that represents that
   connection.
